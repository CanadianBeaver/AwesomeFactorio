...
Производство смазочной жидкости `Lubricant` вообще не представляет проблем. Одного химического завода `!Chemical plant` хватит даже для базы на 75 исследовательских пакетов в минуту. Строить отдельную цистерну для хранения производимой смазочной жидкости тоже нет смысла, в трубах до сборочных автоматов места хватит. Остальное, как например серная кислота `Sulfuric acid` или сера `Sulfur` следует производить на месте, там где они используются, поскольку требуют траспортировки ресурсов на конвейерах.
