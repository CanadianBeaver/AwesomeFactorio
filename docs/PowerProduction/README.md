...
| | | | 500 `!Solar panel` | 420 `!Accumulator` | 21 | Солнечная энергия |
| 450 `!Solid fuel` | 50% `!Transport belt` | 1 `!Offshore pump` | 50 `!Boiler` | 100 `!Steam engine` | 90 | Максимум на насосах |
| | | | | | ~350 | Итого |

Получается, что нам нужно производить полтора конвейера твёрдого топлива (900+450), что тоже не мало. Есть также смысл рассмотреть [улучшение первой паровой электростанции](UpgradingSteamPower.md).
