...
* умножаем на ноль всё что шевелится по близости `!Biter` `!Spitter`
...
* по возвращении строим нефтеперерабатывающий заводик `!Oil refinery`
...
Оптимальное соотношение подбирается в ручную и зависит от вашей производственной ситуации. Можно только дать определённые наставления. Например, самое простое соотношение является 8 нефтеперерабатывающих заводов `!Oil refinery` и 9 химических заводов `Chemical plant`, а именно:

> 8 `Advanced oil processing`\
> 2 `Heavy oil cracking`\
> 7 `Light oil cracking`

Также имеет место и более точное соотношение:

> 20 `Advanced oil processing`\
> 5 `Heavy oil cracking`\
> 17 `Light oil cracking`

Эти цифры весьма условные, они подходят для расчёта производства всего в нефтяной газ `Petroleum gas`. В реальности, есть смысл обращать внимание на [калькулятор](https://kirkmcdonald.github.io/) и на [контролируемое производство с остатками](../CircuitNetwork/SimpleExamples.md#управление-производством-нефти).

Для сжижения угля картина примерно следующая:

> 12 `Coal liquefaction`\
> 8 `Heavy oil cracking`\
> 11 `Light oil cracking`

или более точное соотношение:

> 60 `Coal liquefaction`\
> 39 `Heavy oil cracking`\
> 55 `Light oil cracking`

## Использование модулей

Модули разные бывают, модули разные будут полезны. В *Factorio* так устроено, что и нефтеперерабатывающие заводы `Oil refinery` и химические заводы `Chemical plant` имеют по три свободных слотов для модулей и не имеют градации по уровням, как не в пример у сборочных автоматов. А это значит, что у нас могут возникнуть траблы, особенно с переходом базы из 45 исследовательских пакетов в 75, простое усовершенствование невозможно. Можно конечно поиграться модулями скорости `!Speed module 3`, но вариант так себе. Планируйте свободное место для размещения требуемых нефтеперерабатывающих и химических заводов при улучшении начальной базы.
...
Большая база, маленькие производства. Не пытайтесь строить нефтеперерабатывающий завод производящий всё, так же как на начальной базе. Тут лучше воспользоваться принципом "*одно нечто делает только одно шота*". Исключение составит смазочная жидкость `Lubricant`, которую придётся комбинировать с производством попутного газа или ракетного топлива. Производить отдельно твёрдое топливо не нужно. То есть, строим множество мелких нефтеперерабатывающие заводы по следующим правилам:

* всё в попутный газ
* всё в ракетное топливо
* всё в смазочную жидкость и остатки в попутный газ или в ракетное топливо

И неважно какой входной ресурс вы используете, будь то нефть или сжижение угля. Стоит учитывать, чтобы запустить производство через сжижение угля, нужно доставить небольшое количество мазута `Heavy oil`. Это можно сделать дронами, хватит и пары бочек. Также, есть смысл производить пар `Steam` для сжижения угля комбинируя привозимый уголь и производимое твёрдое топливо, отдавая приоритет твёрдому топливу. А ещё лучше запустить отдельное производство пара ядерным реактором `Nuclear reactor` и доставлять пар вагон-цистернами `Fluid wagon`.
