# Знаки на постоянных комбинаторах

Откровенно говоря, постоянные комбинаторы `Constant combinator` не предназначены чтобы их использовали в качестве поэтического забора. Тем не менее иногда бывает полезно разместить некую пометку или текст на всякий случай. Делается это при помощи виртуальных сигналов на постоянном комбинаторе, которые включают буквы английского алфавита и цифры, знаков препинания не предусмотрено. Плюсом является то, что постоянные комбинаторы не зависимы от линии электропередач. Писать можно и на других комбинаторах, но они будут жрать энергию почем зря и занимают по две клетки.

## Настройка отображения для комбинаторов

Но при игровых настройках по умолчанию сигналы комбинаторов не отображаются. Для отображения сигналов нужно зайти в настройки `Settings`, затем `Interface` и установить галочку для показа настроек комбинатора в *Alt-mode*:

![Настройки для комбинаторов](./images/Writing.01.png)

Соответственно, играть придётся тоже в *Alt-mode*, но это на самом деле единственно нормальный режим игры, хотя кому как. Далее задаём на постоянном комбинаторе любые виртуальные сигналы и любуемся видами:

![Отображение для комбинаторов](./images/Writing.02.png)
...
Это позволит легко ориентироваться в глухой застройке ваших макаронных фабрик и позволит находить на карте сундуки для пополнения предметов в рюкзаке `Toolbelt`, особенно до появления транспортных дронов `Logistic robot`. Помечать можно не только сундуки, но и всё что сочтёте важным в игре.
...
## Многолетние дэбилы из *Wube Software*

Далее описываются игровые механики, которые шопорукие чешского происхождения сломали после выхода платного диэлси, хотя ничего не мешало их сохранить. Теперь эти параграфы представляют собой исторический интерес для созерцания того, как хорошо мы жили при коммунизме.
...
https://youtube.com/watch?v=vixfnONzSgU&t=1082
