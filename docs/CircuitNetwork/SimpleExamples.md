...
Создавать логические сети без комбинаторов весело и прикольно, одним сигнальным проводом соединяем два элемента игры и радуемся счастью. Первый подсоединившийся отсылает в сеть своё состояние, режим чтения. Второй присоединившийся как-то реагирует на полученное состояние, режим действия. Например, хотим забирать стальные балки `Steel plate` с конвейера, если их скопилось больше определённого значения:
...
С выходом Factorio второй версии, появилась возможность подключать `!Chemical plant` химические заводы к логической сети, а значить ими можно управлять напрямую, без использования `!Pump` помп для контроля потока жидкости.
...
[*![Простые примеры логических сетей](https://img.youtube.com/vi/vixfnONzSgU/0.jpg)*](https://www.youtube.com/watch?v=vixfnONzSgU)
