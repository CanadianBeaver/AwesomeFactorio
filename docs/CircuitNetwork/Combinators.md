...
В *Factorio* есть любопытная механика, когда несколько сигнальных проводов сходятся на одном объекте, их значения автоматически складываются, даже если провода разных цветов. Это позволяет комбинировать данные из разных частей логической сети и получать итоговые суммарные сигналы. Таким образом можно например загружать определённое количество предметов на конвейер или в грузовой вагон `!Cargo wagon`.

*![Объединение множеств на манипуляторах в Factorio](./images/Combinators.09.webp)*

В этом примере реализуется схема, позволяющая поддерживать заданное количество предметов на конвейере, 30 железных пластин `!Iron plate` и 30 железных шестерёнок `!Iron gear wheel`. Постоянный комбинатор `Constant combinator` определяет эти числа в виде двух соответствующих сигналов с цифрами. Арифметический комбинатор `Arithmetic combinator` считывает текущее количество уже выложенных предметов со всего конвейера и умножает это значение на −1, формируя отрицательный сигнал.

![Объединение множеств на комбинаторах в Factorio](./images/Combinators.07.webp)

Когда сигналы от постоянного и арифметического комбинаторов объединяются, их значения суммируются. В результате получается число предметов, которое необходимо догрузить, чтобы довести фактическое количество на конвейере до заданного. Манипуляторы, на которых формируется итоговый суммарный сигнал от двух комбинаторов, автоматически включаются или отключаются в зависимости от того, требуется ли догрузить недостающие предметы. Когда суммарный сигнал показывает положительное значение, манипуляторы активируются и начинают выгружать предметы на конвейер, а когда значение становится нулевым (отсутствует) или отрицательным, они отключаются, поскольку требуемое количество уже достигнуто.

![Объединение множеств на манипуляторах в Factorio](./images/Combinators.08.webp)
...
> Ответ: *можно обойтись без комбинаторов и вовсе! Вот это поворот...*
...
Без комбинаторов не получиЦа [переводить ядерные электростанции в резерв](../PowerProduction/BackupSteamPower.md#с-теорией-всё-начинаем-творить), это когда солнечных панелей `Solar panel` и аккумуляторных блоков `Accumulator` хватает. Так же, без комбинаторов не получиЦа [управлять загрузкой урановых топливных элементов](../PowerProduction/NuclearPower.md#начнём-с-теории) `Uranium fuel cell` в реактор.
...
### Случай второй, погрузка и разгрузка локомотивов
...
С другой стороны, да ваше пофиг. Просто добавь побольше поездов, освой побольше месторождений и влипи балансеров на конвейеры, усё. Все равно, рано или поздно придётся осваивать новые месторождения и запускать новые паровозы. Про форпосты, кто вообще напрягается? Фауна и загрязнение тормозят изрядно игру. Да и бегать их истреблять это ещё тот геморрой, многие таки играют без сего удовольствия, просто вырубают фауну в настройках карты. Да и для снабжения форпостов можно обойтись и без комбинаторов, фильтрация ячеек на хабар в вагонах вам в помощь. Разгрузка хабара на форпостах тоже без проблем, хватит и сигнальных проводов.

### Случай третий, заводик снабжения на одном сборочном автомате

В новом *Factorio* появилась возможность подключать сборочные автоматы и химические заводы к логической сети и управлять текущими заданиями на производство предметов, то есть динамическим менять рецептуру. Возможность, сулившая в начале кучу плюшек, обернулась разочарованием. Реализация этой игровой механики настолько идиотская, что сделать что-то реально применимое в игре не представляется возможным. Заводики снабжения получаются медленно работающими, занимаемое пространство не сильно экономиться по сравнению с обычными заводиками снабжения, а потребляемое электричество от 100500 комбинаторов обслуживающих такие чертежи сравнима обычными заводами снабжения. [Сравните сами](../HowToStartNewGame/Mall.md#магазин-шота-у-ашота).

### Другие симптомы?

Других клинических случаев, где нужны комбинаторы, не наблюдается. В *Factorio* [полно разных задротов](../Additionals/NerdsVsGeeks.md#кого-в-игре-больше-нердов-или-гиков) готовые с пеной у рта и до расшибленного лба доказывать, что дескать комбинаторы дюже нужны везде, вся база должна состоять из одних комбинаторов. Психов полно, однако. За исключением парочки мелких [усовершенствований, которые можно сделать и без комбинаторов](../CircuitNetwork/SimpleExamples.md) больше рассказывать и не о чём.
...
[*![Переводим паровые и атомные электростанции в резерв](https://img.youtube.com/vi/FfmslzAyOsc/mqdefault.jpg)*](https://www.youtube.com/watch?v=FfmslzAyOsc&t=42s)

[*![Плавим ресурсы](https://img.youtube.com/vi/e3mbmKIWLns/mqdefault.jpg)*](https://www.youtube.com/watch?v=e3mbmKIWLns&t=659s)
