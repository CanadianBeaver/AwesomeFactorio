...
Команда энта просто отключает появление нового загрязнения, но не убирает уже накопившееся. В довесок почистить все игровые карты (планеты) от имеющегося загрязнения можно так:
...
Существует несколько модификаций для игры *Factorio* связанные с загрязнением и которые развлекают не по-детски. Самое-самое что стоит попробовать конечно же *[Pollution is the Solution](https://mods.factorio.com/mod/Pollution_is_the_Solution)*. Данная модификация уничтожает улья, если они разродились шестьюдесятью единицами местной фауны. Вот так, загрязнение плачевно влияет на природу.
