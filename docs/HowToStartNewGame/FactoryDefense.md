...
После постройки нефтеперерабатывающего завода начинается самая интересная часть. Новые микросхемы `Advanced circuit` и сера `Sulfur` дают возможность запустить производство голубых склянок, которые `Chemical science pack`. А также открывается светлый путь к открытию летающих дронов, которые сами строят и сами что-то приносят в рюкзак. А после прорыва в робототехнику `Robotics` мы осваиваем производство жёлтых научных склянок `Utility science pack` и понимаем, что ресурсов `!Iron ore` `!Copper ore` `!Coal` на всё не хватает. А чтобы добыть ещё больше ресурсов, нужно опять теснить соседей. А значить наступает время опять нести демократию на кончике стального инструмента `Steel axe`. Начинается вторая экпансия.
